...
export class Solver {
    ...
    printAnswer() {
        console.log(`point: ${this.#answer.point}`);
        console.log(`direction:\nx: ${this.#answer.p.vel.x}, y: ${this.#answer.p.vel.y}`);
        console.log(`outOfBounds: ${this.#answer.outOfBounds ? 'true' : 'false'}`);
    }

    getAnswerSteps() {
        this.#sim.updatePieceState(this.#answer.p);
        this.#sim.runWithStep(this.#answer, this.#answerWithSteps);
        const result = this.#answerWithSteps;
        return result.map(ps => ps.map(p => p.clone()));
    }
